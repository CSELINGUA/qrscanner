...
import com.example.AttendanceSystem.models.Attendant
...
class FireStoreAuthClass {
    ...
    /*fun registerUser(activity: StylistRegister, stylist: Stylist) {
        mFireStoreClass.collection(Constants.USERS).document(stylist.id)
            .set(stylist, SetOptions.merge()).addOnSuccessListener {
                saveUserLocally(activity, stylist)
...
/*    fun completeUserRegistration(activity: Activity, userHashMap: HashMap<String, Any>) {
    ...
    fun getCurrentUser(activity: Activity) {
        ...
        mFireStoreClass.collection(Constants.USERS).document(userId).get().addOnSuccessListener { doc ->
            val user = doc.toObject(Attendant::class.java)!!
...
/*    fun logOut(activity: Activity) {
    ...
    private fun saveUserLocally(activity: Activity, attendant: Attendant) {
        val sharedPreferences = activity.getSharedPreferences(Constants.APP_PREFERENCES, Context.MODE_PRIVATE)
        val editor : SharedPreferences.Editor = sharedPreferences.edit()
        editor.putString(Constants.LOGGED_IN_USERNAME, "${attendant.name} ${attendant.name}")
        editor.apply()
    }
